...
var countedBuses = new Array();

const Scenario1Buses = () => {
  ...
  useEffect(() => {
    // Function to get random interval between 200ms and 740ms
              ...
              <button 
                ...
                onClick={() => {
                  if (isClickable) {
                    ...
                    const randomTimeout = Math.floor(Math.random() * (2500 - 1000 + 1)) + 1000;
                ...
                <img src="/bus.png" style={{ width: "100%", height: "100%" }}></img>
              </button>
              ...
              <motion.div
                ...
                onUpdate={(latest) => {
                  ...
                  if (latest.x >= endOfTrack - 3) {
                    latest.zIndex = 0;
                    latest.display = "none";
                    if (!countedBuses.includes(car)) {
                      const randomPeople = Math.floor(Math.random() * (82 - 12 + 1)) + 12;  // Random between 12-82 for buses
                      setPeopleMoved(prev => prev + randomPeople);
                      countedBuses.push(car);
                    }
                  } else {
                    countedBuses = countedBuses.filter(c => c !== car);
                  }
                }}
                onAnimationComplete={() => {
