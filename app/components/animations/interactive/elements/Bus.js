...
//   const GAP_BETWEEN_CARS = 120;
  ...
  const { addMovedPeopleBus, addNumberOfBuses } = useInteractive();
    ...
    const animate = () => {
      ...
      if (positionRef.current >= trackConstants.END_OF_TRACK) {
        if (!hasBeenCounted.current) {
          ...
          addMovedPeopleBus(randomPeople);
          addNumberOfBuses(1);
  ...
  return (
    <AnimatePresence mode="sync" key={`discarded-${car}`}>
      ...
      >
        <img src={`${basePath}/car.png`} alt="Car" style={{ width: "100%", height: "100%" }} />
      </motion.div>
    </AnimatePresence>
  );
}; 