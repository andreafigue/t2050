...
const MapRoute: React.FC = () => {
  ...
  const getRoutesCurrentOnly = async (
    ...
    const timePart = userDepartTime || departAtTime || "17:00"; // prefer user input
    ...
    const urlTraffic =
      ...
      console.log("Fetching route with URL:", urlTraffic);
            ...
            {routes.length > 0 && (
              <div className="mt-1 ml-1 text-sm text-neutral-700">
                <div>
                  ...
                  {/* Show user-selected Depart At time */}
      {userDepartTime && (
        <div>
          Depart at: {formatTime12h(userDepartTime)}
        </div>
      )}
                {peakTime != null && (
                  <div>
