...
const Population: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/county-data');
        ...
        .then(rows => {
          //console.log("Loaded from CSV:", rows.length, "rows");
          processCountyData(rows);
        })
        .catch(err => {
    ...
    const processCountyData = (rows: CountyCsvRow[]) => {
      const grouped: Record<string, CountyCsvRow[]> = {};
      rows.forEach(r => {
        if (!grouped[r.county]) grouped[r.county] = [];
        grouped[r.county].push(r);
      ...
      //console.log("Parsed countyData keys:", Object.keys(grouped));
      //console.log("Sample transformed:", transformed["King"] || transformed[selectedCounty]);
