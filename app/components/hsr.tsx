...
const TravelComparison: React.FC = () => {
  ...
  // Use a ref to store markers so they persist across renders.
  ...
  const updateMaps = async () => {
    ...
    // Add markers for origin & destination on all maps
  ...
  const drawArc = (map, id, start, end, color) => {
    ...
    for (let i = 0; i <= numPoints; i++) {
      const t = i / numPoints;
      const midLat = (start[1] + end[1]) / 2 + Math.abs(start[1] - end[1]) * 0.3;
    ...
    <div style={{ display: "flex", height: "100%" }}>
      <div style={{ width: "20%", padding: "70px 20px 20px 20px" }}>
        <div style={listboxContainerStyle}>
          ...
          <ul style={listboxStyle}>
            {Object.keys(cities).map((city) => {
              const isDisabled = city === destination;
              return (
                <li
                  key={city}
                  style={{
                    ...(origin === city ? selectedStyle : itemStyle),
                    ...(isDisabled ? { opacity: 0.5, pointerEvents: "none" } : {})
                  }}
                  onClick={() => {
                    if (!isDisabled) setOrigin(city);
                  }}
                >
                  {city}
                </li>
        ...
        <div style={listboxContainerStyle}>
          ...
          <ul style={listboxStyle}>
            {Object.keys(cities).map((city) => {
              const isDisabled = city === origin;
              return (
                <li
                  key={city}
                  style={{
                    ...(destination === city ? selectedStyle : itemStyle),
                    ...(isDisabled ? { opacity: 0.5, pointerEvents: "none" } : {})
                  }}
                  onClick={() => {
                    if (!isDisabled) setDestination(city);
                  }}
                >
                  {city}
                </li>
      ...
      <div style={{ display: "flex", width: "80%" }}>
        {["Car", "Train", "HSR"].map((mode) => (
          <div
            key={mode}
            style={{ flex: 1, padding: "10px 10px 0px 10px", position: "relative" }}
          >
            <h2
              style={{
                textAlign: "center",
